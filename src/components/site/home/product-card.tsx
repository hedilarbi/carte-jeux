...
export function ProductCard({
  ...
  return (
    <article
      className={cn(
        "group overflow-hidden rounded-3xl border border-brand-ice/14 bg-brand-navy font-body transition hover:-translate-y-1 hover:border-brand-lavender/45",
        className,
      )}
    >
      <div
      ...
      >
        ...
        <div className="absolute left-3 top-3 z-10 flex flex-wrap gap-1">
          {product.badges.map((badge) => (
            <span
              className={cn(
                "rounded-md border px-2 py-1 font-body text-[10px] font-bold uppercase",
                badgeStyles[badge],
              )}
              key={badge}
            >
      ...
      <div className="p-4">
        <p className="font-body text-[10px] font-bold uppercase text-brand-periwinkle/60">
          {product.platform}
        </p>
        <h3 className="mt-1 min-h-10 font-body text-sm font-bold leading-5 text-brand-lilac">
          {product.name}
        </h3>
        <div className="mt-3 flex items-center gap-2 text-xs text-brand-periwinkle/70">
          ...
          <span className="font-body">({product.reviews})</span>
        </div>
        ...
        <p className="mt-3 flex items-center gap-2 font-body text-[11px] text-brand-lavender">
